...
sub globalLoader{
	...
	foreach my $dcKey (keys %{$ShoppingList->{Shopping}->{Datacore}}){
		...
		$component{$ShoppingList->{Shopping}->{Datacore}->{$dcKey}->{id}}=$ShoppingList->{Shopping}->{Datacore}->{$dcKey}->{content};
	}
	foreach my $gooKey (keys %{$ShoppingList->{Shopping}->{goo}}){
		$names{$ShoppingList->{Shopping}->{goo}->{$gooKey}->{id}}=$gooKey;
		$component{$ShoppingList->{Shopping}->{goo}->{$gooKey}->{id}}=$ShoppingList->{Shopping}->{goo}->{$gooKey}->{content};
	}
	foreach my $minKey (keys %{$ShoppingList->{Shopping}->{Mineral}}){
		$names{$ShoppingList->{Shopping}->{Mineral}->{$minKey}->{id}}=$minKey;
		$component{$ShoppingList->{Shopping}->{Mineral}->{$minKey}->{id}}=$ShoppingList->{Shopping}->{Mineral}->{$minKey}->{content};
	}
	foreach my $PIKey (keys %{$ShoppingList->{Shopping}->{PI}}){
		$names{$ShoppingList->{Shopping}->{PI}->{$PIKey}->{id}}=$PIKey;
		$component{$ShoppingList->{Shopping}->{PI}->{$PIKey}->{id}}=$ShoppingList->{Shopping}->{PI}->{$PIKey}->{content};
	}
}